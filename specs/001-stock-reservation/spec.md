...
- Q: Should the pending-reservation TTL countdown reset when a new item is added? → A: **Yes**, the countdown resets on each add. This is a **configurable** behavior via a `RESET_TTL_ON_ADD` flag: enabled = reset the pending window on each add; disabled = each pending hold keeps its original `creation + TTL` expiration. The reset is scoped to the user's pending holds **for that same item** (per-user, per-item) — adding a hold on one item does not extend holds on other items.
...
4. **Given** an already-confirmed reservation, **When** confirm is sent again, **Then** it is a safe
   no-op returning the same confirmed reservation. Idempotency here is **by reservation state**: the
   hold already carries the idempotency key from its reserve, and confirm sends no new key.
...
- **Confirm an already-confirmed reservation** → safe no-op returning the same confirmed reservation
  (idempotency is by reservation state, not a new key); no double effect.
...
- **TTL reset on add**: with `RESET_TTL_ON_ADD` enabled, adding a new pending hold refreshes the
  pending-window countdown **only for that user's pending holds of the same item** (per-user,
  per-item); holds on other items are untouched. With it disabled, each pending hold keeps its
  original `creation + TTL` expiration. The configured behavior MUST be consistent between the
  backend sweep and the UI clock.
...
- **FR-016**: Users MUST be able to **confirm** a pending reservation, transitioning it from PENDING
  ...
  reservation non-expiring. Confirm references the reservation by its id and carries **no new
  idempotency key** — the reservation already holds the key generated by its original reserve.
  Confirm MUST be idempotent **by reservation state**: re-confirming an already-confirmed reservation
  is a safe no-op returning the same confirmed reservation (HTTP 200), while confirming a reservation
  that is no longer pending because it was released, expired, or never existed MUST be rejected with
  a clear typed result (404 not found / 409 no-longer-pending).
- **FR-017**: System MUST support a configurable `RESET_TTL_ON_ADD` behavior: when enabled, adding a
  new pending reservation resets the pending-window countdown **for that user's pending holds of the
  same item only** (per-user, per-item — other items are unaffected); when disabled, each pending
  hold keeps its original `creation + TTL` expiration. The chosen setting MUST be applied
  consistently by the backend expiration sweep and surfaced to the frontend countdown, and MUST be
  documented.
...
- **SC-008**: A confirmed reservation still exists with its units held after 60+ seconds elapse (it
  does not expire), and confirming the same reservation twice yields exactly one confirmed
  reservation with no extra stock effect (confirm idempotency is by reservation state).
- **SC-009**: With `RESET_TTL_ON_ADD` enabled, adding a new pending hold extends the pending-window
  expiration to a fresh TTL **for the user's pending holds on that same item** (other items
  unaffected); with it disabled, each pending hold expires at its own `creation + TTL`. Backend sweep
  and UI countdown agree on the configured behavior.
...
- **Mutation ownership**: `confirm` and `release` identify the reservation by its UUID and do **not**
  enforce that it belongs to the calling `X-User-Id`. The no-auth model relies on reservation UUIDs
  being unguessable; per-user ownership enforcement on these mutations is out of scope for this
  challenge (not a requirement). The `X-User-Id` header still scopes the read-only "my reservations"
  view.
